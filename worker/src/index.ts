...
// Analyze pronunciation with Claude
app.post('/api/analyze-pronunciation', async (c) => {
  const user = c.get('user');
  if (!user) {
    return c.json({ error: 'Unauthorized' }, 401);
  }
  ...
  const formData = await c.req.formData();
  const file = formData.get('file') as unknown;
  const expectedHanzi = formData.get('expected_hanzi') as string;
  const expectedPinyin = formData.get('expected_pinyin') as string;
  const expectedEnglish = formData.get('expected_english') as string;

  if (!file || typeof file !== 'object' || !('arrayBuffer' in file)) {
    return c.json({ error: 'file is required' }, 400);
  }
  if (!expectedHanzi) {
    return c.json({ error: 'expected_hanzi is required' }, 400);
  }
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        tools: [{
          ...
          description: 'Analyze the pronunciation of a Mandarin Chinese audio recording',
          input_schema: {
            type: 'object',
            properties: {
              transcribed_text: { type: 'string', description: 'The Chinese characters (hanzi) of what you actually hear the speaker saying' },
              transcribed_pinyin: { type: 'string', description: 'The pinyin with tone marks of what you hear' },
              accuracy_score: { type: 'number', description: 'Pronunciation accuracy score from 1-10' },
              is_correct: { type: 'boolean', description: 'Whether the speaker said the expected phrase correctly' },
              feedback: { type: 'string', description: 'Brief feedback on pronunciation, tone errors, or other issues' },
            },
            required: ['transcribed_text', 'transcribed_pinyin', 'accuracy_score', 'is_correct', 'feedback'],
          },
        }],
        tool_choice: { type: 'tool', name: 'analyze_pronunciation' },
        messages: [{
          role: 'user',
          content: [
            {
            ...
            {
              type: 'text',
              text: `Listen to this Mandarin audio. The speaker is trying to say: ${expectedHanzi} (${expectedPinyin || ''}, meaning: ${expectedEnglish || ''}). Rate their pronunciation accuracy 1-10, transcribe what you actually hear them saying, and give brief feedback on any tone or pronunciation errors.`,
            },
          ],
        }],
      }),
    ...
    if (!response.ok) {
      ...
      console.error('[analyze-pronunciation] API error:', response.status, errText);
    ...
    if (!toolUse?.input) {
      return c.json({ error: 'No analysis result' }, 500);
    }
...
app.post('/api/transcribe-google', async (c) => {
  const user = c.get('user');
  if (!user) {
    return c.json({ error: 'Unauthorized' }, 401);
  }
  ...
  if (!c.env.GOOGLE_TTS_API_KEY) {
    return c.json({ error: 'Google API is not configured' }, 500);
  }
  ...
  try {
    const response = await fetch(
      `https://speech.googleapis.com/v1/speech:recognize?key=${c.env.GOOGLE_TTS_API_KEY}`,
      {
        method: 'POST',
    ...
    if (!response.ok) {
      const errText = await response.text();
      console.error('[transcribe-google] API error:', response.status, errText);
      return c.json({ error: 'Google transcription failed' }, 500);
    }
    ...
    console.error('[transcribe-google] Error:', err);
    return c.json({ error: 'Google transcription failed' }, 500);
  }
});

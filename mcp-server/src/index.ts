...
// MCP Server
export class ChineseLearningMCP extends McpAgent<Env> {
  ...
  async init() {
    ...
    this.server.tool(
      ...
      async ({ deck_id }) => {
        ...
        if (!deck) {
          return {
            content: [{ type: "text", text: `Deck not found: ${deck_id}` }],
            isError: true,
          };
        }
    ...
    this.server.tool(
      ...
      async ({ deck_id }) => {
        ...
        // Get all cards with their notes
    ...
    this.server.tool(
      ...
      async ({ note_id }) => {
        ...
        if (!note) {
          return {
            content: [{ type: "text", text: `Note not found: ${note_id}` }],
            isError: true,
          };
        }
    ...
    this.server.tool(
      ...
      async () => {
        const [totalCards, cardsDue, studiedToday, totalDecks] = await Promise.all([
          this.env.DB.prepare('SELECT COUNT(*) as count FROM cards').first<{ count: number }>(),
          this.env.DB.prepare(
          ...
          this.env.DB.prepare('SELECT COUNT(*) as count FROM decks').first<{ count: number }>(),
        ]);

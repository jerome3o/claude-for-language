...
import { getMyCardReviews, getAudioUrl, API_BASE } from '../api/client';
import { Loading, ErrorMessage } from '../components/Loading';
import { useAudioPlayer, useNoteAudio } from '../hooks/useAudio';
import { CardType, RATING_INFO } from '../types';
...
export function MyCardReviewDetailPage() {
  const { date, cardId } = useParams<{ date: string; cardId: string }>();
  const noteAudio = useNoteAudio();

  const cardReviewsQuery = useQuery({
    queryKey: ['myCardReviews', date, cardId],
    queryFn: () => getMyCardReviews(date!, cardId!),
    enabled: !!date && !!cardId,
  });

  if (cardReviewsQuery.isLoading) {
    return <Loading />;
  }
    ...
    <div className="page">
      <div className="container">
        ...
        <div className="review-detail-header">
          <Link to={`/progress/day/${date}`} className="back-link">← Back to Day</Link>

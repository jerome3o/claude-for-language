...
import { getMyDayCards } from '../api/client';
...
export function MyDayDetailPage() {
  const { date } = useParams<{ date: string }>();

  const dayCardsQuery = useQuery({
    queryKey: ['myDayCards', date],
    queryFn: () => getMyDayCards(date!),
    enabled: !!date,
  });

  if (dayCardsQuery.isLoading) {
    return <Loading />;
  }
    ...
    <div className="page">
      <div className="container">
        ...
        <div className="day-header">
          <Link to="/progress" className="back-link">← Back to Progress</Link>
          ...
          {cards.length === 0 ? (
            ...
            <div className="cards-list">
              {cards.map((card) => (
                <Link
                  key={card.card_id}
                  to={`/progress/day/${date}/card/${card.card_id}`}
                  className="card-item"
                >

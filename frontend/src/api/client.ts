...
export async function analyzePronunciation(
  ...
  const formData = new FormData();
  formData.append('file', audioBlob, 'recording.webm');
  formData.append('expected_hanzi', expectedHanzi);
  formData.append('expected_pinyin', expectedPinyin);
  formData.append('expected_english', expectedEnglish);
  ...
  const response = await fetch(`${API_PATH}/analyze-pronunciation`, {
    method: 'POST',
    credentials: 'include',
    headers,
    body: formData,
...
export async function transcribeAudioGoogle(audioBlob: Blob): Promise<TranscriptionResult> {
  const formData = new FormData();
  formData.append('file', audioBlob, 'recording.webm');
  ...
  const response = await fetch(`${API_PATH}/transcribe-google`, {
    method: 'POST',
    credentials: 'include',
    headers,
    body: formData,
  ...
  if (!response.ok) {
    throw new Error('Google transcription failed');
  }

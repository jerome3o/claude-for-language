...
import { createInvite, createStarterDeck } from '../../api/invites';
...
/** Sentinel id for the built-in deck that is created on send. */
const STARTER_OPTION_ID = '__starter__';
const STARTER_DECK_NAME = 'Starter Chinese';
const STARTER_WORD_COUNT = 15;
const WELCOME_PLACEHOLDER = '欢迎！先学这几个词，有问题在这里问我。';
...
/**
 * The happy path for inviting a student: the built-in Starter Chinese deck is
 * ticked for them, they can add their own decks, write a welcome note, tap
 * Create link, then scan or send. Relationship, expiry, multi-use and email
 * binding hide under Options.
 */
export function InviteSheet({ onClose, onCreated }: InviteSheetProps) {
  const { isOnline } = useNetwork();
  const queryClient = useQueryClient();
  const [role, setRole] = useState<RoleChoice>('tutor');
  const [selectedDecks, setSelectedDecks] = useState<Set<string>>(() => new Set([STARTER_OPTION_ID]));
  ...
  const decks = useMemo(() => (decksQuery.data ?? []).filter(d => d.user_id !== null), [decksQuery.data]);

  // If the tutor already has a Starter Chinese deck, the real deck stands in
  // for the built-in option (and is ticked in its place).
  const existingStarter = useMemo(() => decks.find(d => d.name === STARTER_DECK_NAME) ?? null, [decks]);
  useEffect(() => {
    if (!existingStarter) return;
    setSelectedDecks(prev => {
      if (!prev.has(STARTER_OPTION_ID)) return prev;
      const next = new Set(prev);
      next.delete(STARTER_OPTION_ID);
      next.add(existingStarter.id);
      return next;
    });
  }, [existingStarter]);
  ...
  const needsDeck = role === 'tutor';
  const deckCount = selectedDecks.size;
  const canCreate = isOnline && !isCreating && (!needsDeck || deckCount > 0);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (role === 'tutor') {
        shareDeckIds = Array.from(selectedDecks).filter(id => id !== STARTER_OPTION_ID);
      ...
      setCreatingStep('Creating the link…');
      const created = await createInvite({
        inviter_role: role === 'none' ? null : role,
        share_deck_ids: shareDeckIds,
        welcome_message: role === 'none' ? null : (welcome.trim() || null),
  ...
  const sharedCount = invite?.share_deck_ids ? (() => {
    try { return JSON.parse(invite.share_deck_ids!).length as number; } catch { return 0; }
  })() : 0;
    ...
    <div className="invite-sheet-backdrop" onClick={onClose} role="presentation">
      ...
      >
        <div className="invite-sheet-header">
          <h2 id="invite-sheet-title">{invite ? 'Your invite link' : 'Invite a student'}</h2>
        ...
        {!invite ? (
          <div className="invite-sheet-body">
            <p className="invite-sheet-lead">
              They scan this in class or tap the link, sign in with Google, and land straight in
              their first session. No email address needed.
            </p>

            {role === 'tutor' && (
              <div className="invite-field">
                <span className="invite-label">Start them with</span>
                {decksQuery.isLoading ? (
                  <p className="invite-muted">Loading your decks…</p>
                ) : (
                  <div className="invite-deck-list">
                    {!existingStarter && (
                      <label className={`invite-deck-option invite-deck-starter${selectedDecks.has(STARTER_OPTION_ID) ? ' selected' : ''}`}>
                        <input
                          type="checkbox"
                          checked={selectedDecks.has(STARTER_OPTION_ID)}
                          onChange={() => toggleDeck(STARTER_OPTION_ID)}
                        />
                        <span className="invite-deck-text">
                          <span className="invite-deck-name">{STARTER_DECK_NAME}</span>
                          <span className="invite-deck-desc">
                            Built-in · {STARTER_WORD_COUNT} words (你好, 谢谢, 对不起…) · added to your decks when you send
                          </span>
                        </span>
                      </label>
                    )}
                    {decks.map(deck => (
                      <label key={deck.id} className={`invite-deck-option${selectedDecks.has(deck.id) ? ' selected' : ''}`}>
                        ...
                        <span className="invite-deck-text">
                          ...
                          {deck.id === existingStarter?.id && (
                            <span className="invite-deck-desc">Built-in starter deck · copied to them on first sign-in</span>
                          )}
                        </span>
                      </label>
                    ))}
                ...
                <p className="invite-muted">
                  {deckCount === 0
                    ? 'Pick at least one deck — the link needs something for them to study.'
                    : `${deckCount === 1 ? 'This deck is' : `These ${deckCount} decks are`} copied to them the moment they sign in.`}
                </p>
              </div>
            )}
            ...
            {role !== 'none' && (
              <div className="invite-field">
                <label className="invite-label" htmlFor="invite-welcome">
                  Welcome message <span className="invite-label-hint">(optional, sent as your first chat message)</span>
                </label>
                <textarea
                  id="invite-welcome"
                  className="invite-welcome-input"
                  value={welcome}
            ...
            {showOptions && (
              <div className="invite-options">
                <div className="invite-option-row">
                  <span className="invite-option-label">Our relationship</span>
          ...
          <div className="invite-sheet-body invite-result">
            ...
            <p className="invite-muted invite-result-summary">
              ...
              {sharedCount > 0 ? ` and get ${sharedCount} deck${sharedCount === 1 ? '' : 's'} to start with` : ''}
              {invite.welcome_message ? ' — your welcome message is waiting for them' : ''}

...
import { db, getSyncMeta, updateSyncMeta, LocalDeck, LocalNote, PendingReview } from '../db/database';
...
describe('SyncService', () => {
  ...
  describe('syncPendingReviews', () => {
    it('should sync pending reviews to the server', async () => {
      ...
      const pendingReview: PendingReview = {
        ...
        original_queue: CardQueue.NEW,
        new_queue: CardQueue.LEARNING,
        new_learning_step: 1,
        new_ease_factor: 2.5,
        new_interval: 0,
        new_repetitions: 1,
        new_next_review_at: null,
        new_due_timestamp: Date.now() + 600000,
        _pending: true,
        _retries: 0,
        _last_error: null,
      };
      ...
      const result = await syncService.syncPendingReviews();

      expect(result.synced).toBe(1);
      expect(result.failed).toBe(0);

      // Review should be marked as synced (not deleted)
      const reviews = await db.pendingReviews.toArray();
      expect(reviews).toHaveLength(1);
      expect(reviews[0]._pending).toBe(false);
    });

    it('should not sync already-synced reviews', async () => {
      // Add a synced review (not pending)
      ...
      expect(mockFetch).not.toHaveBeenCalledWith(
        expect.stringContaining('/api/study/review'),
        expect.anything()
      );
    });
    ...
    it('should handle sync failures and increment retry count', async () => {
      ...
      // Review should still be pending with incremented retry count
      const review = await db.pendingReviews.get('review-1');
      expect(review?._pending).toBe(true);
      expect(review?._retries).toBe(1);
      expect(review?._last_error).toBe('Server error');
    });

    it('should send reviews in chronological order', async () => {
      // Add reviews in non-chronological order
      const reviews: PendingReview[] = [
        {
          ...
          original_queue: CardQueue.NEW,
          new_queue: CardQueue.REVIEW,
          new_learning_step: 0,
          new_ease_factor: 2.5,
          new_interval: 4,
          new_repetitions: 1,
          new_next_review_at: '2026-01-23T12:00:00Z',
          new_due_timestamp: null,
          _pending: true,
          _retries: 0,
          _last_error: null,
        },
        {
          ...
          time_spent_ms: 5000,
          user_answer: null,
          reviewed_at: '2026-01-19T11:00:00Z', // Earlier
          original_queue: CardQueue.NEW,
          new_queue: CardQueue.LEARNING,
          new_learning_step: 1,
          new_ease_factor: 2.5,
      ...
      mockFetch.mockImplementation(async (url, options) => {
        if (url.includes('/api/study/review')) {
      ...
      // First request should be for the earlier review
      expect(requestBodies[0].card_id).toBe('card-1');
      expect(requestBodies[1].card_id).toBe('card-2');
    });
  });
